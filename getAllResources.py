...
# Add columns
    table.add_column("Resource Name", style="dim", width=25)
    # table.add_column("Resource ID")
    table.add_column("Plan Name")
    table.add_column("Billable Units [Metrics / Unit]")
    ...
    for resource in resources:
        ...
        # Check if plan_id starts with "classic_infrastructure" and skip it if true
            ...
            cost = plan['cost']
            rounded_cost = str(round(cost, 4))
        
            billable_units_list = [f"{usage['metric']}/{usage['unit']}" for usage in plan['usage']]
            billable_units = ", ".join(billable_units_list)
    
            table.add_row(resource_name, plan_name, billable_units, rounded_cost)
